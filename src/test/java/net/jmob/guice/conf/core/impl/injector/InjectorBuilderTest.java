/**
 * Copyright 2015 Yves Galante
 * <p/>
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * <p/>
 * http://www.apache.org/licenses/LICENSE-2.0
 * <p/>
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package net.jmob.guice.conf.core.impl.injector;

import com.typesafe.config.ConfigException;
import net.jmob.guice.conf.core.BindConfig;
import net.jmob.guice.conf.core.InjectConfig;
import org.junit.Test;

import java.util.Optional;
import java.util.stream.Stream;

import static org.hamcrest.core.Is.is;
import static org.junit.Assert.assertThat;

@BindConfig(value = "rootPath")
public class InjectorBuilderTest {


    @Test
    public void noProperty () {
        Stream<Injector> build = new InjectorBuilder(NoProperty.class).build();
        assertThat(build.count(), is(0L));
    }

    @Test
    public void oneProperty () {
        Stream<Injector> build = new InjectorBuilder(OneProperty.class).build();
        assertThat(build.count(), is(1L));
    }

    @Test(expected = ConfigException.class)
    public void invalidPath () {
        new InjectorBuilder(InvalidPath.class).build();
    }

    @BindConfig(value = "rootPath")
    public static class NoProperty {

    }

    @BindConfig(value = "rootPath")
    public static class OneProperty {
        @InjectConfig
        private Optional<String> properties;
    }

    @BindConfig(value = "rootPath", path="subPath")
    public static class InvalidPath {
    }
}
